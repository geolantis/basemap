...
// HL layers to add on top - but using a DIFFERENT source name
const hlLayers = [
  {
    "id": "AUSTRIA_HL_Gesamt_gen20cm_smooth20m_HL/10",
    "type": "line",
    "source": "esri-hl",  // Changed to esri-hl
...
// Create new style keeping all original sources and layers, adding new HL source and layers
const updatedStyle = {
  ...originalBasemap,
  ...
  sources: {
    ...originalBasemap.sources,  // Keep ALL original sources including esri
    // Add NEW source specifically for HL layers
    "esri-hl": {
      "type": "vector",
      "tiles": [
...
console.log(`- Original sources preserved: ${Object.keys(originalBasemap.sources).length}`);
console.log(`- New HL source added: esri-hl`);
