-- Migration: Add Layer Groups System (Final Version)
...
-- ============================================================================
-- PART 5: Add category validation triggers (if map_category exists)
-- ============================================================================

-- Create trigger functions to validate categories if map_category column exists
CREATE OR REPLACE FUNCTION check_basemap_category()
RETURNS TRIGGER AS $$
DECLARE
  category TEXT;
  has_category_column BOOLEAN;
BEGIN
  -- Check if map_category column exists
  SELECT EXISTS (
  ...
  IF has_category_column THEN
    -- Get the category of the basemap
    SELECT map_category INTO category
    FROM map_configs
    WHERE id = NEW.basemap_id;

    -- Check if it's a background map
    IF category IS NOT NULL AND category != 'background' THEN
      RAISE EXCEPTION 'Basemap must be of category "background", got "%"', category;
    END IF;
  END IF;
...
CREATE OR REPLACE FUNCTION check_overlay_category()
RETURNS TRIGGER AS $$
DECLARE
  category TEXT;
  has_category_column BOOLEAN;
BEGIN
  ...
  IF has_category_column THEN
    -- Get the category of the overlay
    SELECT map_category INTO category
    FROM map_configs
    WHERE id = NEW.overlay_id;

    -- Check if it's an overlay map
    IF category IS NOT NULL AND category != 'overlay' THEN
      RAISE EXCEPTION 'Overlay must be of category "overlay", got "%"', category;
    END IF;
  END IF;
...
-- Create triggers to validate categories
CREATE TRIGGER validate_basemap_category
  BEFORE INSERT OR UPDATE OF basemap_id ON layer_groups
  FOR EACH ROW
  EXECUTE FUNCTION check_basemap_category();

CREATE TRIGGER validate_overlay_category
  BEFORE INSERT OR UPDATE OF overlay_id ON layer_group_overlays
  FOR EACH ROW
  EXECUTE FUNCTION check_overlay_category();
...
-- All users can view all layer groups (for authenticated requests)
CREATE POLICY "All users can view layer groups"
  ON layer_groups FOR SELECT
  USING (true);
...
-- ============================================================================
-- PART 12: Grant permissions (simplified for your setup)
-- ============================================================================

-- Grant usage on tables to public for reading
GRANT SELECT ON layer_groups TO PUBLIC;
GRANT SELECT ON layer_group_overlays TO PUBLIC;
GRANT SELECT ON layer_group_tags TO PUBLIC;
GRANT SELECT ON layer_group_tag_assignments TO PUBLIC;
GRANT SELECT ON layer_groups_complete TO PUBLIC;
GRANT SELECT ON layer_group_overlays_detail TO PUBLIC;

-- Grant execute on functions to public
GRANT EXECUTE ON FUNCTION get_layer_group_overlays(UUID) TO PUBLIC;
GRANT EXECUTE ON FUNCTION check_layer_compatibility(UUID, UUID) TO PUBLIC;
