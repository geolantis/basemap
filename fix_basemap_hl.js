...
// Read the original basemap8.json
const originalBasemap = JSON.parse(fs.readFileSync('/Users/michael/Development/basemap/basemap8.json', 'utf8'));

// Create the structure you provided with HL layers
const hlStyle = {
  "version": 8,
  ...
  "sources": {
    // Keep the original basemap-source for the raster layer
    "basemap-source": originalBasemap.sources["basemap-source"],
    // Update the esri source to use vector tiles for HL layers
    "esri": {
      "type": "vector",
      "tiles": [
...
// First, add the raster basemap layer
if (originalBasemap.layers && originalBasemap.layers.length > 0) {
  // Find and add the basemap-layer first
  const basemapLayer = originalBasemap.layers.find(l => l.id === "basemap-layer");
  if (basemapLayer) {
    hlStyle.layers.push(basemapLayer);
  }
}

// Then add the HL layers on top
const hlLayers = [
  {
...
// Add all HL layers
hlStyle.layers = hlStyle.layers.concat(hlLayers);

// Write the result - directly replace basemap8.json
fs.writeFileSync('/Users/michael/Development/basemap/basemap8.json', JSON.stringify(hlStyle, null, 2));

console.log('Successfully updated basemap8.json with HL layers on top');
console.log('Structure:');
console.log('- Raster basemap layer (background)');
console.log(`- ${hlLayers.length} HL layers (isolines and labels) on top`);
console.log(`Total layers: ${hlStyle.layers.length}`);